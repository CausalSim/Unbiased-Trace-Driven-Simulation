...
def load_and_create_datasets(dict_exp, policy_out):
    ...
    savepath = f"train_data_{file_name}/"
    try:
...
def main():
    ...
    train_causal_sim(
        ...
        models_path=f"models/{args.policy_out}",
    )
    ...
    cf_causalsim, features_causalsim = generate_cfs(
        dict_exp,
        generated_datapath,
        models_path=f"models/{args.policy_out}/{alg}/",
    ...
    print("TRAIN SLSIM .. ")
    # train direct
    train_slsim(generated_datapath, models_path=f"models/{args.policy_out}")
